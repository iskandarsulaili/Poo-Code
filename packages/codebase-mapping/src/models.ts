...
export function createMappingEvent(type: MappingEvent["type"], data: Record<string, unknown> = {}): MappingEvent {
...
export function createLogger(level: CodebaseMappingConfig["logLevel"]) {
	...
	return {
		debug: (...args: unknown[]) => {
			if (currentLevel <= 0) console.debug("[codebase-mapping:debug]", ...args)
		},
		info: (...args: unknown[]) => {
			if (currentLevel <= 1) console.info("[codebase-mapping:info]", ...args)
		},
		warn: (...args: unknown[]) => {
			if (currentLevel <= 2) console.warn("[codebase-mapping:warn]", ...args)
		},
		error: (...args: unknown[]) => {
			if (currentLevel <= 3) console.error("[codebase-mapping:error]", ...args)
		},
	}
}
