...
export class ASTParser {
	...
	private parseWithRegex(language: Language, content: string, filePath: string): SyntaxNode {
		...
		let nodeId = 0
		const _accumulated: AccumulatedImport[] = []
		...
		for (let lineIdx = 0; lineIdx < lines.length; lineIdx++) {
			...
			if (/^(import|const)\s*[{[(]/.test(trimmed) && !trimmed.includes("}")) {
				...
				let accName = ""
				const accKind = "import_statement"
				let accText = line
				let endLine = lineIdx
				...
				for (let lookIdx = lineIdx + 1; lookIdx < lookEnd; lookIdx++) {
					...
					if (/[}\]]/.test(lookLine) && resolvedPath) {
						...
						const _namedImports = trimmed.match(/{\s*([^}]+)\s*}/)
						break
					}
					// Reached a from/require without bracket close — treat as import
					if (
						(/from\s+["']/.test(lookLine) || /require\(["']/.test(lookLine)) &&
						!/from\s+["'][^"']+["']/.test(accText)
					) {
				...
				if (accName) {
					const range = createSourceRange(
						...
						content.indexOf(lines[endLine] || "") + (lines[endLine] || "").length,
					)
			...
			if ((/import\(["']/.test(trimmed) || /dynamic\s+import\(["']/.test(trimmed)) && !trimmed.includes("from")) {
				...
				if (dynMatch) {
					...
					children.push(
						createSyntaxNode(id, "import_statement", `dynamic import ${dynMatch[1]}`, range, language),
					)
				}
			}
			...
			if (reExportMatch) {
				...
				if (reExportName) {
					...
					children.push(
						createSyntaxNode(id, "import_statement", `re-export ${reExportName}`, range, language),
					)
				}
			}
		}
	...
	private getPatternsForLanguage(language: Language): Array<{ kind: string; regex: RegExp }> {
		...
		const universal: Array<{ kind: string; regex: RegExp }> = [
			...
			{ kind: "import_statement", regex: /(?:import|use|require|include|from)\s+["']?([\w.\-/]+)/ },
			{
				...
				regex: /(?:export|module\.exports|pub\s+(?:fn|struct|enum|trait|fn|mod))\s+(?:default\s+)?(?:function|class|const|let|var|fn|struct|enum)?\s*(\w+)?/,
			},
		]
